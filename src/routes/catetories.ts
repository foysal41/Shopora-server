...
function categoryErrorStatus(error: unknown) {
  if (!(error instanceof CategoryError)) return 500;
  if (error.code === "not-found") return 404;
  if (error.code === "duplicate" || error.code === "has-products") return 409;
  return 400;
}

function sendCategoryError(res: Parameters<Parameters<typeof router.get>[1]>[1], error: unknown, fallback: string) {
