...
import {
  searchTikTokEmails,
} from "../services/tiktokEmailScraper.js";
...
router.post(
  ...
  async (req, res) => {
    try {
      const body =
        req.body as Partial<TikTokEmailScraperInput>;
      ...
      const input: TikTokEmailScraperInput =
        {
          keywords:
            cleanedKeywords,
      ...
      console.log(
        `TIKTOK EMAIL SEARCH - USER: ${req.user?.id}`
      );
      ...
      return res.status(200).json({
        success: true,
        message:
          "TikTok email leads fetched successfully.",
        data: leads,
        meta: {
          total: leads.length,
          keywords:
      ...
      console.error(
        "TIKTOK EMAIL LEADS ROUTE ERROR:",
        error
      );
