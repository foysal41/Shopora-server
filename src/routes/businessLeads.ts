...
import {
  searchBusinessLeads,
} from "../services/businessLeads.js";

import type {
  BusinessLeadSearchInput,
} from "../types/businessLeads.js";
...
router.post(
  ...
  async (req, res) => {
    try {
      const body =
        req.body as Partial<BusinessLeadSearchInput>;
      ...
      if (
        !Array.isArray(searchStringsArray) ||
      ...
      if (cleanedSearchStrings.length === 0) {
        return res.status(400).json({
          success: false,
          message:
            "At least one valid search keyword is required",
        });
      }
      ...
      if (
        typeof maxCrawledPlacesPerSearch !==
          "number" ||
        !Number.isInteger(
          maxCrawledPlacesPerSearch
        )
        ...
        return res.status(400).json({
          success: false,
          message:
            "maxCrawledPlacesPerSearch must be an integer",
        });
      }
      ...
      if (maxCrawledPlacesPerSearch < 10) {
        return res.status(400).json({
          success: false,
          message:
            "Minimum number of results is 10",
        });
      }
      ...
      const input: BusinessLeadSearchInput = {
        searchStringsArray:
          cleanedSearchStrings,
        locationQueries:
          cleanedLocations,
        maxCrawledPlacesPerSearch,
        extractContactsFromWebsite:
      ...
      console.log(
        `BUSINESS LEAD SEARCH - USER: ${req.user?.id}`
      );
      ...
      const leads =
        await searchBusinessLeads(input);
      ...
      return res.status(200).json({
        success: true,
        message:
          "Business leads fetched successfully",
        data: leads,
        meta: {
          total: leads.length,
          searchStringsArray:
