import type {
  FacebookPost,
  ProductInsightPost,
  ProductInsightsAnalysis,
  ProductInsightsInput,
  ProductInsightSentiment,
...
const APIFY_ACTOR_URL =
  "https://api.apify.com/v2/acts/coregent~facebook-posts-scraper/run-sync-get-dataset-items";
...
const fetchFacebookPosts = async (
  ...
  if (!response.ok) {
    let errorMessage =
      "Facebook post scraper request failed.";
  ...
  if (!Array.isArray(data)) {
    console.error(
      "INVALID FACEBOOK SCRAPER RESPONSE:",
      data
    );
...
const normalizePosts = (
  ...
  const normalizedPosts: ProductInsightPost[] =
    [];
  ...
  for (const post of posts) {
    ...
    normalizedPosts.push({
      ...
      reactions:
        typeof engagement.reactions ===
        "number"
          ? engagement.reactions
          : 0,

      comments:
        typeof engagement.comments ===
        "number"
          ? engagement.comments
          : 0,

      shares:
        typeof engagement.shares ===
        "number"
          ? engagement.shares
          : 0,
...
const isSentiment = (
  value: unknown
): value is ProductInsightSentiment => {
  return (
    value === "positive" ||
    value === "negative" ||
    value === "neutral"
...
const analyzePostsWithAI = async (
  ...
  productQuery: string
): Promise<ProductInsightsAnalysis> => {
  if (!posts.length) {
    return {
      overallSentiment: {
        positive: 0,
...
`;
  ...
  for (
    const item of completeAnalyzedPosts
  ) {
    if (
      item.sentiment ===
      "positive"
  ...
  return {
    ...
    commonPositiveFeedback:
      getStringArray(
        parsed.commonPositiveFeedback,
        7
      ),

    commonNegativeFeedback:
      getStringArray(
        parsed.commonNegativeFeedback,
        7
      ),
