...
interface AIProductShareContent {
  hook: string;
  caption: string;
  hashtags: string[];
}

const parseAIResponse = (rawContent: unknown): AIProductShareContent => {
  let content = "";
  ...
  if (!hook || !caption || hashtags.length === 0) {
    throw new Error(
      "AI did not generate complete share content."
    );
  }
...
export const generateProductShareContent = async (
  productId: string,
  sellerId: string
): Promise<AIProductShareContent> => {
  /*
   * =========================================================
   * FIND PRODUCT
   * =========================================================
  ...
  if (product.sellerId !== sellerId) {
    throw new Error(
      "You do not have permission to generate content for this product."
    );
  }
