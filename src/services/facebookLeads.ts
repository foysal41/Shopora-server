import { FacebookLeadSearchInput } from "../types/facebookLeads.js";

const APIFY_API_URL =
  "https://api.apify.com/v2/actors/memo23~facebook-search-scraper/run-sync-get-dataset-items";
...
export const searchFacebookLeads = async (
  ...
  if (!ALLOWED_SEARCH_TYPES.has(input.searchType)) {
    throw new Error(
      "Invalid Facebook search type."
    );
  }

  const maxItems = Math.min(
    Math.max(input.maxItems || 20, 1),
    200
  );
