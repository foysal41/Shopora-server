...
export const scrapeFacebookAds = async (
  ...
  const response = await fetch(
    `https://api.apify.com/v2/actors/${APIFY_ACTOR_ID}/run-sync-get-dataset-items`,
    {
      ...
      body: JSON.stringify({
        ...
        includeAboutPage:
          input.includeAboutPage,

        isDetailsPerAd:
          input.isDetailsPerAd,
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Apify request failed: ${responseText.slice(
        0,
        500
      )}`
    );
