...
export async function sales(period, filters) {
...
export async function inventory(filters) {
    ...
    const [summary, stockStatus, lowStock, outOfStock, uncategorized, inventoryValue] = await Promise.all([
        ...
        prisma.$queryRawUnsafe(`SELECT COUNT(*)::int AS count FROM "Product" ${categoryClause} ${filters.categoryId ? "AND" : "WHERE"} "stockQuantity" > 0 AND "stockQuantity" <= "lowStockAlert"`, ...productParams),
        prisma.product.count({ where: { ...productWhere, stockQuantity: { lte: 0 } } }),
        prisma.product.count({ where: { ...productWhere, CategoriesId: null } }),
        prisma.$queryRawUnsafe(`SELECT COALESCE(SUM("stockQuantity" * COALESCE("salePrice", "regularPrice")), 0) AS value FROM "Product" ${categoryClause}`, ...productParams),
    ]);
...
export async function sellers(filters) {
    const { where, params } = sqlFilters(filters, true);
...
export async function categories(filters) {
    const { where, params } = sqlFilters(filters, true);
