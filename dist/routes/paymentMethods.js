...
function validateCreate(body) {
    ...
    if (!nonEmptyString(data.cardholderName))
        return { message: "cardholderName is required" };
    if (!nonEmptyString(data.billingAddress))
        return { message: "billingAddress is required" };
    if (!nonEmptyString(data.providerRef))
        return { message: "providerRef is required" };
    ...
    if (!supportedBrands.has(brand))
        return { message: "A supported card brand is required" };
...
router.patch("/:id", async (req, res) => {
    ...
    if ("label" in body)
        allowed.label = body.label === null ? undefined : body.label.trim();
    if ("billingAddress" in body)
        allowed.billingAddress = body.billingAddress.trim();
