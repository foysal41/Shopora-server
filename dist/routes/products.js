...
const normalizeText = (value) => value
    ...
    .split(/\s+/)
    .filter(Boolean);
const buildSimilarityScore = (referenceTokens, productText) => {
...
router.post("/visual-search", (req, res) => {
    ...
    upload.single("image")(req, res, async (error) => {
        ...
        try {
            ...
            const fileNameTokens = Array.from(new Set(normalizeText(file.originalname || "")
                .filter((token) => token.length > 2 && !["png", "jpg", "jpeg", "webp"].includes(token))));
            const referenceTokens = Array.from(new Set([
            ...
            const rankedProducts = candidateProducts
                ...
                .filter((product) => product.similarity > 0.08)
...
router.post("/:id/ai-share", requireAuth, requireSellerProductAccess, async (req, res) => {
    try {
        ...
        const content = await generateProductShareContent(productId, userId);
        return res.status(200).json({
            success: true,
        ...
        console.error("AI PRODUCT SHARE ERROR:", error);
