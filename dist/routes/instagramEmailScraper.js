...
import { searchInstagramEmails, } from "../services/instagramEmailScraper.js";
const router = Router();
/**
...
router.post("/", requireAuth, requireSellerProductAccess, async (req, res) => {
    try {
        ...
        console.log(`INSTAGRAM EMAIL SEARCH - USER: ${req.user?.id}`);
        // ==========================================
        // APIFY SEARCH
        // ==========================================
        const leads = await searchInstagramEmails(input);
        // ==========================================
        // RESPONSE
        // ==========================================
        return res.status(200).json({
            success: true,
            message: "Instagram email leads fetched successfully.",
            data: leads,
            meta: {
                total: leads.length,
                keywords: cleanedKeywords,
        ...
        console.error("INSTAGRAM EMAIL LEADS ROUTE ERROR:", error);
