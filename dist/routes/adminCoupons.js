...
router.get("/", async (_req, res) => {
    try {
        res.json({ success: true, message: "Coupons fetched successfully", data: await getCoupons(true) });
    }
    catch (error) {
...
router.post("/", async (req, res) => {
    try {
        res.status(201).json({ success: true, message: "Coupon created successfully", data: await createCoupon(req.body, req.user.id) });
    }
    catch (error) {
...
router.patch("/:id", async (req, res) => {
    try {
        res.json({ success: true, message: "Coupon updated successfully", data: await updateCoupon(String(req.params.id), req.body) });
    }
    catch (error) {
